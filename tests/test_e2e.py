...
def test_end_to_end(client: TestClient):
    ...
    # 7) reviewer approves (requires at least one attachment for DEFAULT)
    files = {"file": ("evidence.txt", b"dummy", "text/plain")}
    r = client.post(f"/applications/{app_id}/attachments", files=files, headers=auth_header(operator_token))
    assert r.status_code == 200, r.text
